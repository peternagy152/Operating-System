...
struct Command commands[] =
{
		...
		{ "ver", "Print the FOS version" ,command_ver},
		{ "add", "Add two integers" ,command_add},
		{ "rum", "reads one byte from specific location" ,command_readmem},	//need arguments

		//Assignment1 commands
		//====================
		{"ip", "Check if the given string is palindrome or not", command_palindrome},

		//Assignment1.BONUS command
		//=========================
		{ "xcc", "Execute set of concatenated commands separated by /",command_xcc}, //need no arguments
...
int execute_command(char *command_string)
{
	...
	char WordEntered[len];
	strcpy(WordEntered,arguments[0]);
	...
	if (command_found == 0)
	{
		...
		{
				int command_len = strlen(commands[j].name);
				char commandname[command_len];
				strcpy(commandname , commands[j].name);
				int k = 0 ;
				...
				if(len > command_len)
				{
					...
					{
						if(WordEntered[k] != commandname[k])
							break;
					}
	...
	if(command_found)
	{
		...
		if (!CheckNumOfArgs(arguments, number_of_arguments))
		{
			cprintf("Invalid number of arguments for command: %s\n", arguments[0]);
...
/*ASSIGNMENT-1*/
//========================================================
//Q1:IsPalindrome Command	(0.5 MARK)
/*DON'T change this function*/
int command_palindrome(int number_of_arguments, char **arguments )
{
	//DON'T WRITE YOUR LOGIC HERE, WRITE INSIDE THE IsPalindrome() FUNCTION
	uint32 ret = IsPalindrome(arguments);
	if (ret)
		cprintf("yes\n");
	else
		cprintf("no\n");
...
/*---------------------------------------------------------*/
/*FILL this function
 * arguments[1]: string to be checked
 * Return:
 * 		1: if it's palindrome
 * 		0: if it's NOT palindrome
 */
uint32 IsPalindrome(char** arguments)
{
	int len = strlen(arguments[1]);
		char word[len] ;
		strcpy(word , arguments[1]);
		char revword[len];
		int temp = len - 1 ;
		for (int i = 0 ;i < len ; i++)
		{
...
/*---------------------------------------------------------*/
//Q2: Check Number of Arguments (0.75 MARK)
/*FILL this function
 * arguments[0]: command name
 * arguments[1...numOfArgs-1]: other arguments (if any)
 * Return:
 * 		1: if number of arguments the given command is CORRECT
 * 		0: if number of arguments the given command is WRONG
 */
uint32 CheckNumOfArgs(char** arguments, int numOfArgs)
{
  ...
  int len = strlen(arguments[0]);
  char com[len];
  strcpy(com , arguments[0]);
// help Command
if(strncmp(com , "help" , 4) == 0 )
{
	if(arguments[1] != NULL)
		return 0;
}
else if (strncmp(com , "kernel_info" , 11) == 0)
{
	if(arguments[1] != NULL)
			return 0;
	else
...
///This command should allow the user to enter a set of commands (max 20 commands), in ONE line & separated by "/", and execute them after pressing ENTER.

/*DON'T change this function*/
int command_xcc(int number_of_arguments, char **arguments )
{
...
void ExecuteCommands()
{
	//Assignment1.BONUS
	//put your logic here
	//should allow the user to enter a set of commands (max 20 commands), in ONE line & separated by "/", and execute them after pressing ENTER.
